...
const { read, readdir, task, stat, cleanGlob, flatten, verbose } = require('./util');
const {
  SUCCESS,
...
function* getTopLevelDir(dir) {
  ...
  const stats = yield files.map(
    (file) => getStats(dir, file),
  );
...
function getStats(dir, file) {
  ...
  return stat(filePath)
    .then((statFile) => ({ statFile, file, type: SUCCESS }));
}
